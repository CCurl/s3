...
/*  ?  */ void fKey() { c = fgetc(stdin); PUSH = (c != EOF) ? c : 0; }
...
/*  f  */ void fFloat() { u = stb[p++]; // printf("-flt:%c-",u);
    ...
    else if (u == 'R') { PUSH=0; if (NOS) fread((void*)&TOS, 1, 1, (FILE*)NOS); }
    else if (u == 'W') { if (TOS) { fwrite((void*)&NOS, 1, 1, (FILE*)TOS); } s -= 2; } }
/* h */ void fHex() { PUSH=0; while (1) {
            if (btw(stb[p],'0','9')) { TOS=(TOS*16)+stb[p++]-'0'; }
            else if (btw(stb[p],'A','F')) { TOS=(TOS*16)+stb[p++]-'A'+10; }
            else if (btw(stb[p],'a','f')) { TOS=(TOS*16)+stb[p++]-'a'+10; }
            else { return; } } }
/*  i  */ void fRegInc() { u = stb[p++]; 
        if (btw(u, 'A', 'Z')) { st.i[u+32]++; } 
        else if (btw(u, '0', '9')) { st.i[lb+u-'0']++; } 
        else { --p; ++TOS; } }
/*  l  */ void fLoc() { u=stb[p++]; if (u=='+') { lb += (lb < 185) ? 10 : 0; }
            else if (u=='-') { lb -= (125 < lb) ? 10 : 0; } }
/*  m  */ void fMOp() { u=stb[p++]; if (u=='@') { TOS=*(char*)TOS; } } // else if (u=='!') { *(char*)TOS=(char)NOS; s-=2; } }
/*  n  */ void fIndex() { PUSH = R0; }
/*  q  */ void fDotS() { for (int i = sb; i <= s; i++) { printf("%c%ld", (i == sb) ? 0 : 32, st.i[i]); } }
/*  r  */ void fRegGet() { u = stb[p++]; PUSH=0;
        if (btw(u, 'A', 'Z')) { TOS=st.i[u+32]; } 
        else if (btw(u, '0', '9')) { TOS=st.i[lb+u-'0']; } }
/*  s  */ void fRegSet() { u = stb[p++]; t=POP;
...
/*  x  */ void fExt() { u = stb[p++];
        if (u == '%') { NOS %= TOS; s--; } // MOD
            ...
            PUSH=sd; } 
        else if (u == 'A') { TOS=(TOS<0)?-TOS:TOS; } // ABS
...
/*  z  */ void fZType() { TOS=(long)&stb[TOS]; dotQ(0); }
...
void (*q[128])() = { 
    ...
    fSys,X,fBit,fCOp,fRegDec,fExec,fFloat,X,fHex,fRegInc,X,X,fLoc,fMOp,fIndex,X, // 96:111
    X,fDotS,fRegGet,fRegSet,fType,fUser,X,X,fExt,X,fZType,fBegin,fQt,fWhile,fLNot,X }; // 112:127
